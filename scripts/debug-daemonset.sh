...
pod_name=$(kubectl get pods --no-headers -o custom-columns=":metadata.name" | grep '^certificate-installer-' | head -n 1)

if [ -z "$pod_name" ]; then
  echo "No pod found with the name starting with 'certificate-installer-'"
  exit 1
fi
